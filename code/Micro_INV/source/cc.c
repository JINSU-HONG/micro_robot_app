...
#pragma CODE_SECTION(SPI_RXINT, "ramfuncs");
#pragma CODE_SECTION(cc, "ramfuncs");
#pragma CODE_SECTION(AD_Process, "ramfuncs");
...
//    delaycc(1e-6);
    ...
    if (Flag.OverallMode == OVERALL_MODE_INIT)
    {
        ...
        if (SPI_DAT_FILL == false)
        {
            ...
            SPI_data_TX[0] = 0xAAAA; // connection test
            SPI_data_TX[1] = 0x5555; // connection test
            SPI_data_TX[2] = 0x0000; // dummy
            SPI_DAT_FILL = true;
        }
        SwitchOff();
        ResetHINV();
    ...
    else if (Flag.OverallMode == OVERALL_MODE_INV_TEST)
    {
        ...
        if (SPI_DAT_FILL == false)
        {
            ...
            {
                SPI_data_TX[0] = 0x1009; // current fault / voltage fault / dummy / inv status
            }
            else if (Flag.VoltageFault)
            {
                SPI_data_TX[0] = 0x0109; // current fault / voltage fault / dummy / inv status
            }
            else
            {
                SPI_data_TX[0] = Flag.ModeStatus; // current fault / voltage fault / dummy / inv status
            }
            SPI_data_TX[1] = 0x0000; // dummy
            SPI_data_TX[2] = 0x0000; // dummy
            SPI_DAT_FILL = true;
        }
    }
            ...
            || Flag.OverallMode == OVERALL_MODE_RUN)
    { // follow given current reference
        if (Flag.ModeStatus == MODE_STATUS_CHECKING
                || Flag.ModeStatus == MODE_STATUS_OK)
        {
            Flag.ModeStatus = MODE_STATUS_OK;
        ...
        if (!SPI_DAT_FILL)
        {
            ...
            if (Flag.CurrentFault)
            {
                SPI_data_TX[0] = 0x1009;
            }
            else if (Flag.VoltageFault)
            {
                SPI_data_TX[0] = 0x0109;
            }
            else
            {
                SPI_data_TX[0] = Flag.ModeStatus;
            }
            SPI_data_TX[1] = temp_sum_h;
            SPI_data_TX[2] = temp_sum_l;
...
//		Ia_present = ScaleAin_adc_A[0]*(double)((int)(AdcaResultRegs.ADCRESULT0&Mode_16bit) - (int)OffsetAin_adc_A[0] );   // ADCA_SOC0 : INVa_shunt_ADC_P/N
    ...
    if (Flag.OverallMode == OVERALL_MODE_INV_TEST && INV.Ls_iden == 0.)
    {
        ...
        else if (INV.iden_cnt > 1000)
        { // coil connection problem
            Flag.ModeStatus = MODE_STATUS_INV_FAULT;
            INV.iden_duty = 0.;
            Flag.Ready = 0;
            Flag.CC_mode = 1;
...
void HPwmUpdate(void)
{
    if (Flag.DTS)
    {
        ...
        if (EPwm2Regs.TBSTS.bit.CTRDIR == TB_UP)
        { // on-sequence at next cc loop
            if (INV.Ib < 0)
            {
                INV.CompP_DB = 0;
