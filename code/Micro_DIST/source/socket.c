   ...
   if ( len > 0 )
   {
      ...
      {                                            // -----------------------------
         case Sn_MR_UDP :                          // UDP mode
            ...
            addr[1] = (uint8)(head[0] & 0xFF);
            addr[2] = (uint8)(head[1] >> 8);
            addr[3] = (uint8)(head[1] & 0xFF);
   ...
   *port = head[2];                       // destination port number
   ...
   if (0) {
       ...
       if (Flag.OverallMode == OVERALL_MODE_INIT) {
		   data_len = UDP_HEADER_LEN + INIT_RECV_LEN;
	   } else if (Flag.OverallMode == OVERALL_MODE_INV_TEST) {
		   data_len = UDP_HEADER_LEN + INV_TEST_RECV_LEN;
	   } else if (Flag.OverallMode == OVERALL_MODE_IDLE) {
		   data_len = UDP_HEADER_LEN + IDLE_RECV_LEN;
	   } else if (Flag.OverallMode == OVERALL_MODE_RUN) {
		   data_len = UDP_HEADER_LEN + RUN_RECV_LEN;
	   }
       wiz_read_buf(s, buf, data_len); // data copy.
       */
   }
