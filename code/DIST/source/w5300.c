...
void     getSHAR(uint8 * addr)
{
   ...
   addr[1] = (uint8)(IINCHIP_READ(SHAR) & 0xFF);
   addr[2] = (uint8)(IINCHIP_READ(SHAR2)>>8);
   addr[3] = (uint8)(IINCHIP_READ(SHAR2) & 0xFF);
   addr[4] = (uint8)(IINCHIP_READ(SHAR4)>>8);
   addr[5] = (uint8)(IINCHIP_READ(SHAR4) & 0xFF);
}
...
{
	uint16 temp;
	temp = IINCHIP_READ(GAR);
	addr[0] = (uint8)(temp>>8);
    addr[1] = (uint8)(temp & 0xFF);
    temp = IINCHIP_READ(GAR2);
	addr[2] = (uint8)(temp>>8);
    addr[3] = (uint8)(temp & 0xFF);
}
...
void     getSIPR(uint8 * addr)
{
	...
	addr[1] = (uint8)(IINCHIP_READ(SIPR) & 0xFF);
	addr[2] = (uint8)(IINCHIP_READ(SIPR2)>>8);
	addr[3] = (uint8)(IINCHIP_READ(SIPR2) & 0xFF);
}
...
uint32   wiz_read_buf(SOCKET s, uint8* buf,uint32 len)
{
      ...
      IINCHIP_CRITICAL_SECTION_ENTER();
//      for (idx = 0; idx < len; idx+=2) {
      for (idx = 0; idx < len/2; idx+=1) {
	  	 if (check_header_flag == 1) {
		    check_header_flag = 0;
            tmp_read = IINCHIP_READ(Sn_RX_FIFOR(s));
			if (tmp_read == HEAD_FIRST_DATA) {
			   *((uint16*)(buf+idx)) = tmp_read;
			   continue;
			} else {
			   *((uint16*)(buf+idx)) = HEAD_FIRST_DATA; 
			   //idx--;
			   continue;
			}
		 }
...
void  wait_1us(uint32 us)
{
   ...
   for(i = 0; i < us ; i++)
   {
	      //for(j = 0; j < 100; j++); // for 100 MHz CPU
	      for(j = 0; j < 200; j++); // for 200 MHz CPU
   }
}
