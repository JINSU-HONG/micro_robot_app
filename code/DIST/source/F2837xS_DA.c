...
//	SpiaRegs.SPICTL.all =0x0006;    		     // Enable master mode, normal phase,
	...
	GpioDataRegs.GPBCLEAR.bit.GPIO61=0x1;  //ncs  low active
     ...
     SpibRegs.SPICCR.all = 0x0047;			//char length 8bit		// Clock polarity = 1
     SpibRegs.SPICCR.bit.HS_MODE = 0x1;
     SpibRegs.SPICTL.all = 0x000E;			// Master mode, Enable transmission, Phase = 0
     SpibRegs.SPISTS.all = 0x0000;
     SpibRegs.SPIBRR.all = 0x002F;				//LSPCLK/(4) = 25MHz
     SpibRegs.SPIPRI.bit.FREE = 1;

     SpibRegs.SPIFFTX.all= 0xC008;			// Enable FIFO's, set TX FIFO level to 8
     SpibRegs.SPIFFRX.all= 0x001F;			// Set RX FIFO level to 8
